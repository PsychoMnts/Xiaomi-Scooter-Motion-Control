...
void setup() {
  ...
  //Decalaring throttle pin as output
  pinMode(led_pin, OUTPUT);
  analogWrite(throttlepin, vthrottleval);
...
bool release_throttle(void *) {
  ...
  analogWrite(throttlepin, 10); //Keep throttle open for 10% to disable KERS. 
...
if (S20C00HZ65.brake > 130) {
  ...
  analogWrite(throttlepin, 0); //close throttle directly when break is touched. 0% throttle
...
// Check if auto throttle is off and speed is increasing
 if (motionstate==motionready) {
   ...
   // speed is increasing
      ...
      analogWrite(throttlepin, 254); //  100% throttle
      timer_m.every(5000, release_throttle); //Set timer to release throttle
